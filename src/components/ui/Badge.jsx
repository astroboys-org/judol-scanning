...
export default function Badge({color="blue", className="", children }) {
    ...
    return (
        <span className={`inline-flex justify-center items-center gap-1 ${colorClasses} text-xs font-medium text-center tracking-wide rounded-full px-2.5 py-0.5 cursor-default ${className}`}>
            {children}
        </span>
    )
}
