    ...
    return (
        <header className="sticky top-0 flex w-screen bg-white border-gray-200 z-[99] dark:border-gray-800 dark:bg-gray-900 lg:border-b">
            <div className="flex flex-col items-center justify-between grow lg:flex-row lg:px-6">
                ...
                <div className={`${isApplicationMenuOpen ? "flex opacity-100" : "hidden opacity-0"} absolute top-0 right-2 lg:relative items-center justify-between gap-4 bg-white dark:bg-gray-700 dark:lg:bg-transparent rounded-2xl lg:rounded-none border border-gray-200 dark:border-gray-800 lg:border-none shadow-md lg:shadow-none w-[50vw] max-w-60 lg:w-full mt-18 lg:mt-0 px-5 py-4 lg:flex lg:opacity-100 lg:justify-end
                    transition transition-discrete duration-100 ease-linear`}>
                    <div className="flex justify-between lg:justify-end items-center gap-3 xs:gap-4 w-full">                        
                        <div className="aspect-square">
                            <ThemeToggleButton />
                        </div>

                        {/* {location.pathname !== "/login" &&
                            <Button color="gray" onClick={() => {navigate("/login"); toggleApplicationMenu();}}>
                                Log In
                            </Button>
                        } */}
                    </div>
