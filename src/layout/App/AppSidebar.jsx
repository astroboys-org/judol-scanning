...
import { CalendarIcon, ChevronDownIcon, EllipsisHorizontalIcon, HomeIcon, ListBulletIcon, PaperAirplaneIcon, TableCellsIcon, UserCircleIcon } from "@heroicons/react/24/outline";
...
const navItems = [
    ...
    },
    {
        ...
        subItems: [{ name: "Menu3", path: "/menu3" }],
    },
    {
        name: "Menu4",
        icon: <TableCellsIcon className="size-6" />,
        subItems: [{ name: "Menu4", path: "/menu4" }],
    },
    {
        name: "Menu5",
        icon: <PaperAirplaneIcon className="size-6" />,
        subItems: [{ name: "Menu 5", path: "/menu5" }],
    },
];
...
function NavItem({navItem, location, checkActive}) {
    ...
    useEffect(() => {
        if (navItem.subItems?.some(item => checkActive(item.path))) {
            setIsOpen(true);
            setIsActive(true);
        }
